...
class OuraClient:
    ...
    def __init__(self, adapter: str = BLE_ADAPTER, auth_key: Optional[bytes] = None):
        ...
        self.probe_events_received = 0  # Events received in current batch
    ...
    async def connect(self, address: Optional[str] = None, timeout: float = 30.0, retries: int = 3) -> bool:
        ...
        # Check if device is already connected via BlueZ (from bluetoothctl or previous session)
        # If so, disconnect first so Bleak can connect properly
        if address:
            try:
                import dbus
                bus = dbus.SystemBus()
                ...
                if is_connected:
                    print(f"Ring already connected via BlueZ, disconnecting first...")
                    device_iface = dbus.Interface(device_obj, "org.bluez.Device1")
                    device_iface.Disconnect()
                    await asyncio.sleep(0.5)
            except Exception:
                pass  # D-Bus check failed, continue normally
    ...
    async def _notification_handler(self, sender, data: bytes):
        ...
        # Event data (0x11 response) - this is the metadata/status packet, not actual events
            ...
            # Track events for batch-based fetch
    ...
    async def get_data(self, start_seq: int = 0, max_events: int = 0,
        ...
        Uses streaming mode (max_events=0) for fastest transfer (~1150 events/sec).
        Ring streams all events continuously until bytes_left=0.

        Args:
            start_seq: Starting sequence number (0 = start from beginning)
            max_events: Max events per request (0 = stream all, fastest mode)
        ...
        # Send single GetEvent command with max_events=0 for streaming mode
        # Ring will stream all events continuously (~1150 events/sec)
        import time
        start_time = time.time()

        cmd = build_get_event_cmd(start_seq, max_events)
        await self.send_command(cmd, f"GetEvent(seq={start_seq}, stream)")

        # Wait for streaming to complete (bytes_left=0)
        last_count = 0
        stall_count = 0
        ...
        while not self.fetch_complete:
            ...
            print(f"  {elapsed:.0f}s: {current} events ({rate:.0f}/sec), bytes_left={self.bytes_left}")
        ...
        elapsed = time.time() - start_time
        print(f"Received {len(self.event_data)} events in {elapsed:.1f}s")
...
async def main():
    ...
    parser.add_argument('--get-all-data', action='store_true', help='Get ALL stored data (streaming mode, fastest)')
    parser.add_argument('--get-data', action='store_true', help='Get stored data with filters (batch mode)')
    parser.add_argument('--output', '-o', default='native_parser/input_data/ring_events.txt', help='Output file')
    ...
    if not (args.heartbeat or args.capture or args.get_all_data or args.get_data or args.set_auth_key
    ...
    try:
        ...
        if args.heartbeat or args.get_all_data or args.get_data or args.sync_time:
        ...
        if args.heartbeat:
        ...
        elif args.get_all_data:
            # Streaming mode - fastest, gets ALL data
            data = await client.get_data(max_events=0)  # 0 = streaming mode
            ...
            with open(args.output, 'w') as f:
                f.write("# Oura Ring Event Export - ALL DATA\n")
            ...
            # Batch mode - for filtered queries
            event_filter = None
            ...
            # Fetch data with batch mode (max_events=255)
            data = await client.get_data(
                max_events=255,
